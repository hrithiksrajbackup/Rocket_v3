...
export function AtsChecker({ resumeData, onClose }: AtsCheckerProps) {
  ...
  const handleCheck = async () => {
    setIsChecking(true);
    try {
      const compatibility = await checkATSCompatibility(
        resumeData,
        jobDescription
      );
    ...
    <div className="py-4">
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Adding a job description helps us analyze keyword matches and provide
      ...
      {isChecking && (
        <div className="mt-6">
          ...
          <Progress value={45} className="h-2" max={100} />
        </div>
      )}
        ...
        <div className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={cn("text-lg font-bold", getScoreColor(result.score))}
              >
                {result.score}/100
              </span>
            </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button variant="default">Apply Suggestions</Button>
          </div>
        </div>
      )}
    </div>
