...
export function EditorSidebar({ 
  ...
  // Handle changes in personal section
  const handlePersonalChange = (personalData: any) => {
    onDataChange('personal', personalData);
  };
  
  // Handle changes in other sections
  const handleSectionChange = (sectionKey: string, data: any) => {
    onDataChange(`sections.${sectionKey}`, data);
  };

  return (
    <div className="w-[420px] flex-shrink-0 border-r h-full overflow-y-auto flex flex-col bg-background">
      <Tabs defaultValue={activeSection} onValueChange={onSectionChange} className="flex-1 flex flex-col">
        <div className="px-4 py-2 border-b sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <TabsList className="grid grid-cols-5 h-auto p-1">
            <TabsTrigger value="personal" className="py-2 px-3 h-auto data-[state=active]:text-primary data-[state=active]:shadow-none data-[state=active]:bg-muted">
              <User className="h-4 w-4" />
            </TabsTrigger>
            <TabsTrigger value="education" className="py-2 px-3 h-auto data-[state=active]:text-primary data-[state=active]:shadow-none data-[state=active]:bg-muted">
              <GraduationCap className="h-4 w-4" />
            </TabsTrigger>
            <TabsTrigger value="experience" className="py-2 px-3 h-auto data-[state=active]:text-primary data-[state=active]:shadow-none data-[state=active]:bg-muted">
              <Briefcase className="h-4 w-4" />
            </TabsTrigger>
            <TabsTrigger value="skills" className="py-2 px-3 h-auto data-[state=active]:text-primary data-[state=active]:shadow-none data-[state=active]:bg-muted">
              <Code className="h-4 w-4" />
            </TabsTrigger>
            <TabsTrigger value="more" className="py-2 px-3 h-auto data-[state=active]:text-primary data-[state=active]:shadow-none data-[state=active]:bg-muted">
          ...
          <Tabs defaultValue="projects" className="flex-1 flex flex-col">
            <div className="px-4 py-2 border-b bg-muted/40">
              <TabsList className="w-full h-auto p-1 bg-transparent justify-start overflow-x-auto flex space-x-2">
                <TabsTrigger value="projects" className="data-[state=active]:bg-background rounded-md">
                  <Lightbulb className="h-4 w-4 mr-2" />
                  Projects
                </TabsTrigger>
                <TabsTrigger value="certifications" className="data-[state=active]:bg-background rounded-md">
                  <Award className="h-4 w-4 mr-2" />
                  Certifications
                </TabsTrigger>
                <TabsTrigger value="languages" className="data-[state=active]:bg-background rounded-md">
                  <Globe className="h-4 w-4 mr-2" />
                  Languages
                </TabsTrigger>
                <TabsTrigger value="interests" className="data-[state=active]:bg-background rounded-md">
                  <Heart className="h-4 w-4 mr-2" />
                  Interests
                </TabsTrigger>
                <TabsTrigger value="references" className="data-[state=active]:bg-background rounded-md">
            ...
            <TabsContent value="languages" className="p-4 flex-1">
              <LanguagesSection 
                ...
                onChange={(data) => handleSectionChange('languages', data)} 
              />
            </TabsContent>
            ...
            <TabsContent value="interests" className="p-4 flex-1">
              <InterestsSection 
                ...
                onChange={(data) => handleSectionChange('interests', data)} 
              />
            </TabsContent>
            ...
            <TabsContent value="references" className="p-4 flex-1">
              <ReferencesSection 
                ...
                onChange={(data) => handleSectionChange('references', data)} 
              />
            </TabsContent>
          </Tabs>
        </TabsContent>
          ...
          {showAiSuggestions ? (
            ...
            <div className="flex flex-col gap-2">
              <div className="text-sm text-muted-foreground">
                Let AI help you improve your resume content and boost your chances of getting noticed.
              </div>
