...
export function ModernTemplate({ resumeData }: ModernTemplateProps) {
  const { personal, sections, settings }: any = resumeData;
  const accentColor = settings.accentColor || "#2563eb";

  return (
    // <div className="w-full h-screen flex font-sans text-gray-800">
    <div className="w-full min-h-screen flex flex-col md:flex-row text-gray-800 font-sans">
      {/* Sidebar */}
      <div className="w-full md:w-1/3 bg-gray-100 p-8 flex flex-col flex-grow">
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold mb-1">{personal.name}</h1>
          <h2 className="text-lg font-medium" style={{ color: accentColor }}>
            {personal.title}
          </h2>
        </div>
        ...
        {sections.languages.visible && sections.languages.items?.length > 0 && (
          <div className="mb-6">
            ...
            <ul className="space-y-1 text-sm">
              {sections.languages.items.map((language: any) => (
      ...
      {/* Main Content */}
      <div className="w-full bg-white md:w-2/3 p-8 flex flex-col flex-grow">
        {/* Summary */}
        {personal.summary && (
          <div className="mb-6">
            <h3
          ...
          sections.experience.items?.length > 0 && (
            <div className="mb-6">
              ...
              <div className="space-y-6">
                {sections.experience.items.map((experience: any) => (
                  <div
                    key={experience.id}
        ...
        {sections.education.visible && sections.education.items?.length > 0 && (
          <div className="mb-6">
            ...
            <div className="space-y-4">
              {sections.education.items.map((education: any) => (
                <div
                  key={education.id}
                  className="relative pl-6 border-l-2"
                  style={{ borderColor: `${accentColor}40` }}
        ...
        {sections.certifications.visible &&
          sections.certifications.items?.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {sections.certifications.items.map((certification: any) => (
